...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_cmd", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_cmd": ["*.pyi", "py.typed"],
        "tree_sitter_cmd.queries": ["*.scm"],
    },
    ext_package="tree_sitter_cmd",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_cmd/binding.c",
                "src/parser.c",
            ],
            define_macros=[
